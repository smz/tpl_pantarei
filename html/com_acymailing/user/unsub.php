	...
	<?php if($this->config->get('unsub_dispoptions',1)){ ?>
		...
		<?php if(!empty($this->mailid)){ ?>
			<div id="unsublist_div" class="unsubdiv">
				<label class="checkbox inline" for="unsublist"><input type="checkbox" value="1" name="unsublist" id="unsublist" checked="checked"/> <?php echo str_replace(array_keys($this->replace),$this->replace,JText::_('UNSUB_CURRENT')); ?></label>
			</div>
		<?php } else {?>
			<div id="unsuball_div" class="unsubdiv">
				<label class="checkbox inline" for="unsuball"><input type="checkbox" value="1" name="unsuball" id="unsuball" <?php if(empty($this->mailid)) echo 'checked="checked"'; ?> /> <?php echo str_replace(array_keys($this->replace),$this->replace,JText::_('UNSUB_ALL')); ?></label>
			</div>
		<?php } ?>
		</div>
	<?php }else{
